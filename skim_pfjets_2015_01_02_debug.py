...
def get_process(datatype, globaltag):
    ...
    # Good Lumi Json file (currently not applied in skim)
    #if is_data:
    ...
    process.kappatuple.active = cms.vstring(
        'LV', 'TrackSummary', 'VertexSummary', 'PFMET', 'PFJets', 'JetArea', 'HCALNoiseSummary',
    )
    if is_data:
    ...
    jec_customise(process, is_data=is_data)
    #-------------------------------------------------------------------------------
    return process

def jec_customise(process, is_data=True):
    ...
    # process.L1Fastjet = process.ak5PFL1Fastjet

    process.jec = cms.Path(process.ak7PFJetsL1Fast*process.ak5PFJetsL1Fast*process.ak5PFJetsL2L3*process.ak5PFJetsL2L3*process.ak5PFJetsL1FastL2L3Residual*process.ak7PFJetsL1FastL2L3Residual)
    process.schedule.insert(0,process.jec)

#def hlt_customise(process):
#    import HLTrigger.HLTfilters.hltHighLevel_cfi as hlt
#    process.jethlt = hlt.hltHighLevel.clone(
#        HLTPaths = ['HLT_PFJet*'],
#        throw = False
#    )
#    process.hltpath    = cms.Path(process.jethlt)
#    process.schedule.insert(0,process.hltpath)
